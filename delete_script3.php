...
if ($fileId !== null) {
    ...
    $departmentQuery = "SELECT department FROM curriculum WHERE id = $fileId";
    ...
    if ($departmentResult && $departmentResult->num_rows > 0) {
        ...
        $deleteCurriculumQuery = "DELETE FROM curriculum WHERE id = $fileId";
        if ($conn->query($deleteCurriculumQuery) === TRUE) {
            // Perform the deletion in the department table
            $deleteDepartmentQuery = "DELETE FROM department WHERE department = '$departmentValue'";
