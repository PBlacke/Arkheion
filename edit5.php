...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'change_password') {
    ...
    try {
        if (!isset($_POST['admin_password']) || !isset($_POST['new_password'])) {
        ...
        // Get the faculty ID from POST data since it's an AJAX request
        ...
        $id = intval($_POST['faculty_id']); // Convert to integer for safety
        ...
        if (!password_verify($adminPassword, $admin['password'])) {
            $response['message'] = "Admin password is incorrect";
        } 
        ...
        error_log("Password update error for faculty ID $id by admin ID $adminId: " . $e->getMessage());
    }
...
if ($id) {
    // Prepare statement to fetch faculty data
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !isset($_POST['action'])) {
    ...
    if (!empty($updateFields)) {
        // Add the faculty ID to the parameters
        $params[] = $id;
        $types .= 'i';
        ...
        <script>
            swal({
                ...
                text: "No changes were made. Please fill in at least one field to update the profile.",
                icon: "info"
            });
        </script>
        <?php
...
button[type="submit"], .change-password-btn {
...
button[type="submit"]:hover, .change-password-btn:hover {
  ...
  <div class="w3-row-padding">
    ...
    <div class="w3-container w3-card w3-white w3-margin-bottom">
        ...
        <div class="w3-container">
            <h5 class="w3-opacity"><b>Update faculty information</b></h5>
...
<div id="passwordModal" class="modal">
    <div class="modal-content">
        ...
        <h3 style="color: #0c1776;">Set New Password for <?php echo htmlspecialchars($facultyData['first_name'] . ' ' . $facultyData['last_name']); ?></h3>
...
function validatePasswordForm(event) {
    ...
    formData.append('faculty_id', '<?php echo $id; ?>');
